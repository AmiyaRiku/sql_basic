...
SELECT MIN(surface_area)AS'アジアの最小面積' FROM countries GROUP BY continent HAVING continent = 'Asia';


-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area)AS'アジア大陸の表面積' FROM countries GROUP BY continent HAVING continent = 'Asia';



-- 問25
-- 全ての国と言語を表示してください。
SELECT name, language FROM countrylanguages INNER JOIN countries ON code = country_code;


-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name, cities.name, language FROM countrylanguages INNER JOIN countries ON code = countrylanguages.country_code INNER JOIN cities ON code = cities.country_code;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT celebrities.name, c3ountries.name FROM celebrities LEFT JOIN countries ON celebrities.country_code = countries.code;
...
INNER JOIN countrylanguages AS m
ON countries.code = m.country_code
WHERE language = (SELECT language
                  FROM countrylanguages AS s
                  WHERE m.country_code = s.country_code
                  ORDER BY s.country_code DESC
                  LIMIT 1);
